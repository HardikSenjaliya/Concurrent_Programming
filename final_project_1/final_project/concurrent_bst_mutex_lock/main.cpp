...
void insert_node(int key, int value, bst_node *root){
	...
	pthread_mutex_lock(&g_tree_lock); //lock the tree lock first to check valid tree exists
...
void *thread_function(void *params){
	...
	switch(operation){
		...
		case SEARCH:
			LOG(DBG, "Search: thread indexes: %d and %d", index->search_low_index, index->search_upper_index);
		...
		case RANGE:
			LOG(DBG, "Thread Range in the Thread function %d and %d", index->range_lower_index, index->range_upper_index);
...
void spawn_threads(user_input *input){
	...
	switch(operation){
		...
		case RANGE:
			printf("Selected Operation: RANGE\n");
			print_map(range_map); //prints values found in the given range
		break;
